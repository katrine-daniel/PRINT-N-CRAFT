      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-2xl overflow-hidden border border-slate-200 hover:border-brand-cyan hover:shadow-clean-md transition-all duration-300 group"
            >
              <div className="relative h-60 overflow-hidden bg-slate-100">
                ...
                />
                <span className="absolute top-3 left-3 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-white/90 text-slate-900 backdrop-blur-md border border-slate-200">
                  {item.category}
                </span>
              </div>
              ...
              <div className="p-6">
                ...
                <p className="text-xs text-slate-500 mt-1 font-medium">
                  Client: <span className="text-slate-800">{item.client}</span>
                </p>
